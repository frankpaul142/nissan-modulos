...
class Ciudad extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nombre_ciudad, id_provincia', 'required'),
			array('id_provincia', 'numerical', 'integerOnly'=>true),
			array('nombre_ciudad', 'length', 'max'=>200),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_ciudad, nombre_ciudad, id_provincia', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
                    		'concessioners' => array(self::HAS_MANY, 'Concessioner', 'city_id'),
		);
	}
